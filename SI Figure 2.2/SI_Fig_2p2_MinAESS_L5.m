%This program computes the minimum value of a necessary for ATFT to resist invasion by L5given w, e, b
% using fzero and DiffPayAtftL5
...
for iw = 1:length(wVal)                                                     % w varies
    ...
    for ie = 1:maxie
        e = eVal(ie);
        fun = @(a)DiffPayAtftL5(a,b,w,e);                                   % difference in fitness Atft and Allc
        a0 = fzero(fun,a0);
...
for ib = 1:length(bVal)                                                     %bvaries
    ...
    for ie = 1:maxie
        e = eVal(ie);
        fun = @(a)DiffPayAtftL5(a,b,w,e);                                   % difference in fitness
        a0 = fzero(fun,a0);
        minAvalB(ib,ie) = a0;
    end
end
