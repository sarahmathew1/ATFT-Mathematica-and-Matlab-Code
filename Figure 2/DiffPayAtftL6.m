%DiffPayAtftL6- This function calculates the difference in expected
%payoff of an ATFT player interacting with an ATFT player and an L6
...
%
%
function [pay] = DiffPayAtftL6(a,b,w,e)
...
V6 = (1-e)*(-c+w*V)+e*w*(a.*Vf+(1-a).*Vp); 
Va = (1-e)*(-c+w*(a.*Vp+(1-a).*Vf))+w*e*((a.^2+(1-a).^2).*V+a.*(1-a).*(Vf+Vp));
